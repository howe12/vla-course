...
CAMERA_MAP = {
    "image": 0,    # 对应 smolvla_libero 的 observation.images.image
    "image2": 2,   # 对应 smolvla_libero 的 observation.images.image2
}
...
class CameraCapture:
    ...
    def _open_cameras(self):
        for name, idx in self.camera_map.items():
            ...
            if not cap.isOpened():
                raise RuntimeError(f"无法打开相机 {name} (video{idx})")
            # 设置请求分辨率（相机可能不支持，会回退到最近值）
            cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
            cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
            self.caps[name] = cap
            print(f"[camera] 打开 {name} (video{idx})")

...
def vla_predict(data, step_count):
    ...
    目前用简单的正弦波轨迹模拟"VLA 理解了任务指令后的动作序列"。
    """
    # 模拟任务："把末端移到目标方块附近"
    ...
    # 简化的关节空间轨迹（模拟 IK 结果）
    q[0] = 0.3 * math.sin(t * 0.5)  # 基座缓慢旋转
    q[1] = -0.5 + 0.2 * math.sin(t * 0.3)  # 肩部俯仰
    q[2] = -1.0 + 0.3 * math.sin(t * 0.4)  # 肘部
    q[3] = 0.5 * math.sin(t * 0.6)  # 腕部旋转
    q[4] = -0.3  # 腕部俯仰
    q[5] = 0.2 * math.sin(t * 0.7)  # 末端
